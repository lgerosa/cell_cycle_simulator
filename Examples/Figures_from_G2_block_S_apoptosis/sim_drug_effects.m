...
cc_names={'G1' 'S' 'G2' 'M'};
%define uperturbed average time in cell cycle phases (in hours)
...
%define the dose-dependent drug effects on apoptosis
     %G1    S     G2     M
Vap=[papG1; 10; papG2;  papM]; %maximal rate on hill equation 
...
ncells=10000; %size of untreated population at start 
...
phase_value=[0 1 2 3 4];
phase_DNA=[2 2 4 4 4];
%define amount of noise to add to DNA measurement
DNA_sigma=0.2; %for normrnd(mu,sigma)
...
for i=1:length(ss_dist_cell_cycle_DMSP)
    ...
    size_phase=(phase_value(i+1)-phase_value(i))./ncells_phase;
    cell_phase_pos=phase_value(i):size_phase:phase_value(i+1);
...
for k=1:length(drug)
    ...
    size_step=0.01;
    size_step=min([size_step min(min(abs(kT(kT~=0)))) min(min(abs(kA(kA~=0))))]);
    size_step=max([size_step 0.01]);
    ...
    for i=1:length(time_steps)-1
        ...
        for j=1:length(phase_value)-1
            %cell cycle update
            iphase=find((phase_value(j)<=single_cells)  & (single_cells <= phase_value(j+1)));
        ...
        i_division=single_cells>phase_value(end);
        %reset them to G1
        single_cells(i_division)=single_cells(i_division)-phase_value(end);
        ...
        if (~isempty(i_time_mes))
            ...
            DNA_dist=normrnd(interp1(phase_value,phase_DNA,single_cells),DNA_sigma);
            %generate pRB overall distribution
            RB_dist=normrnd(interp1(phase_value_RB,phase_RB,single_cells),RB_sigma);
            %plot one-dimensional DNA  or bi-dimensional DNA-pRb distributions
            if (dim_plot==1)
                %plot one dimensional DNA-pRB   
                histogram(DNA_dist,100,'FaceColor','k','EdgeColor','none');
                hold on;
            elseif (dim_plot==2)
            ...
            for j=1:length(phase_value)-1
                ...
                iphase=find((phase_value(j)<=single_cells)  & (single_cells <= phase_value(j+1)));
                DNA_dist=normrnd(interp1(phase_value,phase_DNA,single_cells(iphase)),DNA_sigma);
                RB_dist=normrnd(interp1(phase_value_RB,phase_RB,single_cells(iphase)),RB_sigma);
                %plot one-dimensional DNA  or bi-dimensional DNA-pRb distributions
                if (dim_plot==1)
                   %plot one dimensional DNA
                   histogram(DNA_dist,100,'FaceAlpha',0.5,'EdgeAlpha',0.5,'FaceColor',colors(j,1:3),'EdgeColor','none');
                   %set DNA limit
                   xlim([1 5]);
                ...
                elseif (dim_plot==3)
                   ...
                   scatter(DNA_dist',RB_dist',2,colors(j,1:3));
                   hold on;
                end    
            end     
...
%hist(interp1(phase_value,phase_DNA,single_cells),100)
%plot([0:0.01:4],interp1(phase_value,phase_DNA,[0:0.01:4]));

%plot the single cell distributions at the measured times
